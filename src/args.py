...
class LorraArguments:
    target_layers: str = field(
        metadata={"help": "Layers for Representation. Layers are seperate by `,` eg: `10,12,14,16,18,20` "})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_refusal_retain: Optional[bool] = field(default=True,
                                               metadata={
                                                   "help": "whether to train on refusal retain set for Llama models"})
    ...
    update_refusal_features: Optional[bool] = field(default=False)
    update_refusal_features_step: Optional[int] = field(default=4)

    coeff_harmful: Optional[float] = field(default=0.5)
    coeff_help: Optional[float] = field(default=0.5)
    coeff_over_refusal: Optional[float] = field(default=0.5)


    classifier_path: Optional[str] = field(default=None)

    retain_topk: Optional[int] = field(default=None)
    deltas: Optional[List[float]] = field(default=None)
    beta: Optional[float] = field(default=None)

    refusal_feature_type: Optional[str] = field(default='constant')

    total_steps: Optional[int] = field(default=None)
    using_adaptive_loss: Optional[bool] = field(default=False)
    init_coeff: Optional[float] = field(default=0.0)