...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        features_list = []
        for lab_name, inp_name, att_name in zip(['labels', 'refusal_labels', 'over_refusal_labels'], ['input_ids', 'refusal_input_ids', 'over_refusal_input_ids'], ['attention_mask', 'refusal_attention_mask', 'over_refusal_attention_mask']):
        ...
        fea = features_list[1]
        features.data["refusal_input_ids"] = fea.data["input_ids"]
        features.data["refusal_labels"] = fea.data["labels"]
        features.data["refusal_attention_mask"] = fea.data["attention_mask"]

        if len(features_list) == 3:
            fea = features_list[2]
            features.data["over_refusal_input_ids"] = fea.data["input_ids"]
            features.data["over_refusal_attention_mask"] = fea.data["attention_mask"]
            features.data["over_refusal_labels"] = fea.data["labels"]
        return features
